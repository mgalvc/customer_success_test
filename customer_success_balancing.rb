...
class CustomerSuccessBalancing
  ...
  def execute
    css = @customer_success.reject { |i| @customer_success_away.include?(i[:id]) }
    
    css.sort_by! { |i| i[:score] }

    customers = @customers.sort_by { |i| i[:score] }

    results = {}
    last_score = 0

    css.each do |c|
      will_attend = customers.select do |customer| 
        customer[:score] <= c[:score] and customer[:score] > last_score
      end
      
      last_score = c[:score]

      score = will_attend.length
      
      if !results.has_key?(score) then
        results[score] = []
      end

      results[score].push(c[:id])
    end

    greater = results.keys.max

    results[greater].length > 1 ? 0 : results[greater][0]
  end
end
